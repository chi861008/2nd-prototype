...
export const initialState = (): PrototypeState => ({ version:1, revision:0, stage:'idle', order:emptyOrder(), slides:structuredClone(slides), focusAt:0, deadline:null, remaining:5000, paused:false, scenario:null, nextAt:null });
...
export type Action =
 ...
 | {type:'focus'; id:string; now:number} | {type:'order'; patch:Partial<Pick<Order,'member'|'invoice'|'discount'>>}
 | {type:'payment'; payment:PaymentEntry} | {type:'removePayment'; id:string} | {type:'resetPayments'} | {type:'cancelPay'}
 | {type:'slides'; slides:MarketingSlide[]};
...
export function scenario(index:number, previous:PrototypeState, now:number):PrototypeState {
 ...
 if(index===8)s.order.lines[2].complimentary=true;
...
export function reduce(s:PrototypeState,a:Action):PrototypeState {
 ...
 switch(a.type){
 case 'new': n={...s,stage:'ordering',order:emptyOrder(),deadline:null,paused:false,remaining:5000,nextAt:null,scenario:null,focusId:undefined};break;
 case 'idle':n={...s,stage:'idle',deadline:null,nextAt:null,paused:false};break;
 case 'reset':n=initialState();break;
 case 'clear':if(editable)n={...s,order:{...emptyOrder(),id:s.order.id,number:s.order.number}};break;
 case 'scenario':n=scenario(a.index,s,a.now);break;
 case 'add':if(editable){const l=newLine(a.index);n={...s,order:{...s.order,lines:[...s.order.lines,l]},focusId:l.id,focusAt:a.now};}break;
 case 'line':if(editable)n={...s,order:{...s.order,lines:s.order.lines.map(l=>l.id===a.line.id?{...a.line,quantity:Math.max(1,Math.min(99,integer(a.line.quantity)))}:l)},focusId:a.line.id,focusAt:a.now};break;
 case 'delete':if(editable)n={...s,order:{...s.order,lines:s.order.lines.filter(l=>l.id!==a.id)}};break;
